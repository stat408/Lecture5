...
## Connect the Dots
```{r,echo=F}
mean_temp <- bike.data %>% group_by(month) %>% summarize(mean_temp = mean(temp)) %>% mutate(month = as.numeric(month))
...
ggplot(bike.data, aes(season,count, color = season)) + geom_jitter(alpha = .01) + geom_violin() + stat_summary(fun.y=mean, geom="point", shape=23, size=2) + labs(title='Average number of bike rentals / hour during the four seasons, where 1 = Jan - Mar', caption = 'Source: Capital BikeShare') + theme_dark()
```
